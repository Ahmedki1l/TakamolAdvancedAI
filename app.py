...
@app.route('/ar/analyze-location', methods=['POST'])
def analyze_location_ar():
    if not request.is_json:
        return jsonify({"error": "يجب إرسال بيانات JSON"}), 400

    data = request.get_json()
    property_type = data.get('propertyType')
    facility_counts = data.get('facilityCounts')
    is_good = data.get('isGood')

    if property_type is None or facility_counts is None or is_good is None:
        return jsonify({
            "error": "الحقول المطلوبة مفقودة",
            "الحقول_المطلوبة": ["propertyType", "facilityCounts", "isGood"]
        }), 400
